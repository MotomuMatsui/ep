...
// Writing Matrix file
void writeMAT(double* (&W), ofstream& ofs, int const& size){

  for(int x = 0; x < size; x ++){
    for(int y = 0; y < size-1; y ++){

      auto score = (W[x*size+y]>0)? -log(W[x*size+y]): 10;
      ofs << score << "\t";
    }
    auto score = (W[(x+1)*size-1]>0)? -log(W[(x+1)*size-1]): 10;
    ofs << score << "\n";
  }
...
void countEP(string const& newick_nj, unordered_map<string, double>& ep){
  
  string newick;

  ifstream ifs(newick_nj); // NJ tree (newick format)
  string line, chr;
  while(getline(ifs, line)){
    newick += line;
  }
